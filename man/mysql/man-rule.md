...
```
    【强制】表必备三字段:id, created_at, updated_at
     ...
     created_at, updated_at 的类型均为 datetime 类型，前者现在时表示主动创建，后者过去分词表示被动更新。
```
...
```
    ...
    正例： 人都在150岁之内，用tinyint unsigned存储（取值范围：0-255）。
...
只有当索引的列顺序和order by子句的顺序完全一致，并且所有列的排序方向都一样时，才能用索引排序。
如果查询需要关联多张表，只有当order by子句引用的字段全部为第一个表时，才能使用索引排序。
```
